#!/usr/bin/env python3
"""
접힌 자세(0°,0°,0°) → 기본자세 → 접힌 자세 테스트.
응답 없는 다리는 자동 제외.

사용법:
    python3 stretch_legs.py [--duration 초] [--steps 단계수] [--mock]
"""
...
def main():
    ...
    for leg in sorted(active):
        ...
        if not result["valid"]:
            print(f"  [SKIP] {leg} IK 실패: {result['error']}")
            continue
    ...
    if not default_angles:
        print("[ERROR] IK 계산 가능한 다리가 없습니다.")
        node._bus.close()
        sys.exit(1)

    folded_angles = {leg: FOLDED_ANGLES for leg in default_angles}
    ...
    try:
        ...
        # 2. 기본자세로 펴기
        interpolate(node, folded_angles, default_angles,
        ...
        # 3. 접힌 자세로 접기
        interpolate(node, default_angles, folded_angles,
                    steps=args.steps, step_delay=step_delay,
                    label="[3] 접힌 자세로 접기")
